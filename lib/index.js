...
class Payworks extends EventEmitter {
  ...
  auth (params, options, callback) {
    let schema = {
      ...
      initial_deferment: joi.string().max(2),
      payments_number: joi.string(),
    ...
    if ((params.payments_number &&
      !planTypes.has(params.plan_type)) ||
      (params.initial_deferment && params.plan_type !== '07')) throw new Error('ValidationError')
